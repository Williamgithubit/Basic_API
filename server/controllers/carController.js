...
export const createCar = async (req, res) => {
    ...
    try {
        const newCar = await db.Car.create({
            ...
            features: features || [],
            imageUrl: imageUrl || `https://source.unsplash.com/random/300x200/?${encodeURIComponent((brand || '') + ' ' + (model || 'car'))}`,
            isAvailable: true,
            rating: 4.5,
            reviews: 0,
            isLiked: false
...
// List all cars with filtering and sorting
export const getCars = async (req, res) => {
    ...
    try {
        ...
        if (minPrice || maxPrice) {
            ...
            if (minPrice) where.rentalPricePerDay[db.Sequelize.Op.gte] = parseFloat(minPrice);
            if (maxPrice) where.rentalPricePerDay[db.Sequelize.Op.lte] = parseFloat(maxPrice);
        }

        // Define sort order
        const order = [];
        if (['name', 'brand', 'rentalPricePerDay', 'rating', 'reviews', 'year'].includes(sortBy)) {
        ...
        const cars = await db.Car.findAll({
            ...
            attributes: [
                ...
                'rentalPricePerDay', 'imageUrl', 'isAvailable',
                'rating', 'reviews', 'seats', 'fuelType', 'location',
                'features', 'isLiked'
            ]
        });
...
export const updateCar = async (req, res) => {
    ...
    try {
        ...
        const allowedFields = [
            'name', 'brand', 'model', 'year', 'rentalPricePerDay', 
            'isAvailable', 'imageUrl', 'seats', 'fuelType', 'location',
            'features', 'isLiked'
        ];
...
export const addReview = async (req, res) => {
    ...
    try {
        ...
        const newRating = ((car.rating * car.reviews) + parseFloat(rating)) / newReviews;
