...
import { useGetRentalsQuery, useGetRentalByIdQuery, useGetCustomerRentalsQuery, useGetCarRentalsQuery, useCreateRentalMutation, useUpdateRentalMutation, useCancelRentalMutation, useCompleteRentalMutation } from '../Rental/rentalApi';
import { showErrorToast, showSuccessToast } from '../utils/apiUtils';
export const useRentals = () => {
    const [selectedRentalId, setSelectedRentalId] = useState(null);
    // RTK Query hooks
    const { data: rentals, isLoading: isLoadingRentals, error: rentalsError, refetch: refetchRentals } = useGetRentalsQuery();
    const { data: selectedRental, isLoading: isLoadingSelectedRental } = useGetRentalByIdQuery(selectedRentalId || 0, {
    ...
    const modifyRental = async (id, rentalData) => {
        try {
    ...
    const cancelRentalById = async (id) => {
        try {
    ...
    const completeRentalById = async (id) => {
        try {
