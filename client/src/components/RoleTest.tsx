...
const RoleTest: React.FC = () => {
  const { user, isAdmin, isOwner, isCustomer, logout } = useAuth();

  const renderRoleContent = () => {
    if (!user) {
      return (
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">Please login to see your role</h2>
          <p className="text-gray-600">You can login as a customer, owner, or admin</p>
        </div>
      );
    }
