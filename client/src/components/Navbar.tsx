...
const Navbar: React.FC<NavbarProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <>
      ...
      <nav ref={navRef} className="fixed top-0 left-0 right-0 bg-blue-600 text-white z-50 shadow-md">
            ...
            {isAuthenticated ? (
              <>
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-md text-sm font-medium cursor-pointer ${activeTab === 'rentals' ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                >
                  My Rentals
                </button>
                <button
              ...
              <>
                <Link 
                  to="/login" 
                  className={`px-3 py-2 rounded-md text-sm font-medium ${isActive('/login') ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                >
                  Login
                </Link>
                <Link 
                  to="/signup" 
                  className={`px-3 py-2 rounded-md text-sm font-medium ${isActive('/signup') ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                >
                  Sign Up
                </Link>
              </>
              ...
              {isAuthenticated ? (
                <>
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-md text-base font-medium ${activeTab === 'cars' ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                  >
                    Available Cars
                  </button>
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-md text-base font-medium ${activeTab === 'rentals' ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                  >
                    My Rentals
                  </button>
                ...
                <>
                  <Link
                    to="/login"
                    onClick={closeMenu}
                    className={`block px-3 py-2 rounded-md text-base font-medium ${isActive('/login') ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                  >
                    Login
                  </Link>
                  <Link
                    to="/signup"
                    onClick={closeMenu}
                    className={`block px-3 py-2 rounded-md text-base font-medium ${isActive('/signup') ? 'bg-blue-700' : 'hover:bg-blue-700'}`}
                  >
                    Sign Up
                  </Link>
                </>
